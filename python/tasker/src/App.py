...
class App:
    ...
    def checkIndexRange(self):
            if len(self.command_args) == 0:
                print("Nie podano indeksu")
                return False
            if not self.command_args[0].isdigit():
                print("Indeks musi być liczbą")
                return False
            if int(self.command_args[0]) >= len(self.tasks) or int(self.command_args[0]) < 0:
